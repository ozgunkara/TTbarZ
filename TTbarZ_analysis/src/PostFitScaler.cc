...
// postFit scaling to read from file
double PostFitScaler::postFitScaling(const std::string& process) const{
    return ( postAndPreFitYields.at(process).at(0).second/ postAndPreFitYields.at(process).at(0).first);
}
